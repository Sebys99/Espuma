document.addEventListener("DOMContentLoaded", function () {
    ...
    function actualizarRosas(chatActual) {
        ...
        // Calcular el aumento de tamaño (30% por cada cambio de rosa)
        const aumentoTamano = 1 + 0.3 * (numeroRosa - 1);
        ...
        rosas.forEach((rosa) => {
            ...
            // En los últimos chats, hacer que las rosas cubran completamente
            if (numeroChat >= totalChats - 1) {
                rosa.style.width = `${nuevoTamano * 2}px`;
            }

            // Dentro de actualizarRosas, después de actualizar las rosas:
            if (numeroChat >= totalChats - 3) {
                rosas.forEach((rosa, index) => {
                    // Mover rosas hacia el centro gradualmente
                    const centroX = 50; // 50% del contenedor
                    const centroY = 50;

                    // Calcular posiciones basadas en el índice
                    let nuevaX, nuevaY;
                    if (index % 2 === 0) {
    ...
    chat.addEventListener('click', function (event) {
        ...
        if (this.getAttribute('data-click-count') === '0') {
            ...
            if (clickY > rect.height / 2) {
                const opcionesAbajo = Array.from(this.querySelectorAll('.opciones'))
                    .some(opcion => {
                        const opcionRect = opcion.getBoundingClientRect();
